...
module.exports = cds.service.impl(async function (srv) {
    ...
    srv.on('login', async (req) => {
        ...
        if (user.status === 'LOCKED') {
            ...
            await UPDATE(Users).set({ status: 'ACTIVE', failedLoginCount: 0, lockedUntil: null }).where({ ID: user.ID });
            user.status = 'ACTIVE';
        }
        ...
        if (!valid) {
            ...
            const update = newCount >= 5
                ? { failedLoginCount: newCount, status: 'LOCKED', lockedUntil: new Date(Date.now() + 30 * 60 * 1000) }
